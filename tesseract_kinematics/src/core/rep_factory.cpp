                                                 ...
                                                 const YAML::Node& config) const
{
  ...
  try
  {
    ...
    if (!config["manipulator_reach"])
      throw std::runtime_error("REPInvKinFactory, manipulator_reach node is missing!");

    m_reach = config["manipulator_reach"].as<double>();

    // Get positioner sample resolution
    if (!config["positioner_sample_resolution"])
      throw std::runtime_error("REPInvKinFactory, positioner_sample_resolution node is missing!");
    ...
    if (!config["positioner"])
      throw std::runtime_error("REPInvKinFactory, positioner node is missing!");

    const YAML::Node& positioner = config["positioner"];
    const YAML::Node& p_config = positioner["config"];
    std::string p_class = positioner["class"].as<std::string>();
    fwd_kin = plugin_factory.createFwdKin(p_class, name + "positioner", scene_graph, scene_state, p_config);
    if (fwd_kin == nullptr)
    ...
    if (!config["manipulator"])
      throw std::runtime_error("REPInvKinFactory, manipulator node is missing!");

    const YAML::Node& manipulator = config["manipulator"];
    const YAML::Node& m_config = manipulator["config"];
    std::string m_class = manipulator["class"].as<std::string>();
    inv_kin = plugin_factory.createInvKin(m_class, name + "manipulator", scene_graph, scene_state, m_config);
