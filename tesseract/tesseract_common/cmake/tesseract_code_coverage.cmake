...
# option of `TESSERACT_CODE_COVERAGE` needs to be set to 'ON', either by GUI, ccmake, or
# on the command line.
#
...
# `tesseract_code_coverage(<TARGET_NAME>)`, where the target is given and thus only
...
# `tesseract_code_coverage(<TARGET_NAME>)` on an *executable* target.
#
...
# tesseract_code_coverage(theExe) # As an executable target, adds the 'ccov-theExe' target (instrumentation already added via global anyways) for generating code coverage reports.
# ~~~
#
...
# ~~~
# add_library(theLib lib.cpp)
# tesseract_code_coverage(theLib) # As a library target, adds coverage instrumentation but no targets.
#
# add_executable(theExe main.cpp)
# target_link_libraries(theExe PRIVATE theLib)
# tesseract_code_coverage(theExe) # As an executable target, adds the 'ccov-theExe' target and instrumentation for generating code coverage reports.
# ~~~
#
...
# tesseract_code_coverage(theExe EXCLUDE non_covered.cpp test/*) # As an executable target, the reports will exclude the non-covered.cpp file, and any files in a test/ folder.
# ~~~
#
...
#
# add_executable(theExe main.cpp non_covered.cpp)
# tesseract_code_coverage(theExe AUTO ALL EXCLUDE non_covered.cpp test/*) # As an executable target, adds to the 'ccov' and ccov-all' targets, and the reports will exclude the non-covered.cpp file, and any files in a test/ folder.
# ~~~
...
if(TESSERACT_CODE_COVERAGE AND NOT TESSERACT_CODE_COVERAGE_ADDED)
  set(TESSERACT_CODE_COVERAGE_ADDED ON)
  ...
  if(CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang"
    ...
    if(NOT LLVM_COV_PATH)
      ...
      if(LLVM_COV_VERSION VERSION_LESS "7.0.0")
        message(
          WARNING
            "tesseract_code_coverage()/add_code_coverage_all_targets() 'EXCLUDE' option only available on llvm-cov >= 7.0.0"
        )
      endif()
    endif()
...
# ~~~
function(tesseract_code_coverage TARGET_NAME)
  # Argument parsing
  ...
  cmake_parse_arguments(
    tesseract_code_coverage "${options}" "${single_value_keywords}"
    "${multi_value_keywords}" ${ARGN})

  if(NOT tesseract_code_coverage_COVERAGE_TARGET_NAME)
    # If a specific name was given, use that instead.
    set(tesseract_code_coverage_COVERAGE_TARGET_NAME ${TARGET_NAME})
  endif()

  # Targets
  get_target_property(target_type ${TARGET_NAME} TYPE)

  if(TESSERACT_CODE_COVERAGE AND NOT ${target_type} STREQUAL "INTERFACE_LIBRARY")
    ...
    if(target_type STREQUAL "SHARED_LIBRARY" AND tesseract_code_coverage_ALL)
      if(CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang"
         OR CMAKE_CXX_COMPILER_ID MATCHES "(Apple)?[Cc]lang")
        add_custom_target(
          ccov-run-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
        ...
        if(NOT TARGET ccov-libs)
          message(
            FATAL_ERROR
              "Calling tesseract_code_coverage with 'ALL' must be after a call to 'add_code_coverage_all_targets'."
          )
        endif()
    ...
    if(target_type STREQUAL "EXECUTABLE")
      if(CMAKE_C_COMPILER_ID MATCHES "(Apple)?[Cc]lang"
        ...
        # add them here
        foreach(SO_TARGET ${tesseract_code_coverage_OBJECTS})
        ...
        add_custom_target(
          ccov-run-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
            LLVM_PROFILE_FILE=${tesseract_code_coverage_COVERAGE_TARGET_NAME}.profraw
            $<TARGET_FILE:${TARGET_NAME}> ${tesseract_code_coverage_ARGS}
          COMMAND echo "-object=$<TARGET_FILE:${TARGET_NAME}>" >>
                  ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/binaries.list
          COMMAND
            echo
            "${CMAKE_CURRENT_BINARY_DIR}/${tesseract_code_coverage_COVERAGE_TARGET_NAME}.profraw "
            >> ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/profraw.list
          DEPENDS ccov-preprocessing ccov-libs ${TARGET_NAME})

        # Merge the generated profile data so llvm-cov can process it
        add_custom_target(
          ccov-processing-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND ${LLVM_PROFDATA_PATH} merge -sparse
                  ${tesseract_code_coverage_COVERAGE_TARGET_NAME}.profraw -o
                  ${tesseract_code_coverage_COVERAGE_TARGET_NAME}.profdata
          DEPENDS ccov-run-${tesseract_code_coverage_COVERAGE_TARGET_NAME})

        # Ignore regex only works on LLVM >= 7
        if(LLVM_COV_VERSION VERSION_GREATER_EQUAL "7.0.0")
          foreach(EXCLUDE_ITEM ${tesseract_code_coverage_EXCLUDE})
        ...
        add_custom_target(
          ccov-show-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
            ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}> ${SO_OBJECTS}
            -instr-profile=${tesseract_code_coverage_COVERAGE_TARGET_NAME}.profdata
            -show-line-counts-or-regions ${EXCLUDE_REGEX}
          DEPENDS ccov-processing-${tesseract_code_coverage_COVERAGE_TARGET_NAME})

        # Print out a summary of the coverage information to the command line
        add_custom_target(
          ccov-report-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
        ...
        add_custom_target(
          ccov-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND
            ${LLVM_COV_PATH} show $<TARGET_FILE:${TARGET_NAME}> ${SO_OBJECTS}
            -instr-profile=${tesseract_code_coverage_COVERAGE_TARGET_NAME}.profdata
            -show-line-counts-or-regions
            -output-dir=${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${tesseract_code_coverage_COVERAGE_TARGET_NAME}
            -format="html" ${EXCLUDE_REGEX}
          DEPENDS ccov-processing-${tesseract_code_coverage_COVERAGE_TARGET_NAME})

      elseif(CMAKE_COMPILER_IS_GNUCXX)
        set(COVERAGE_INFO
            "${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${tesseract_code_coverage_COVERAGE_TARGET_NAME}.info"
        )

        # Run the executable, generating coverage information
        add_custom_target(
          ccov-run-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND $<TARGET_FILE:${TARGET_NAME}> ${tesseract_code_coverage_ARGS}
          DEPENDS ccov-preprocessing ${TARGET_NAME})
        ...
        if(NOT ${tesseract_code_coverage_EXTERNAL})
          set(EXTERNAL_OPTION --no-external)
        endif()

        # Capture coverage data
        add_custom_target(
          ccov-capture-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
          COMMAND ${CMAKE_COMMAND} -E remove ${COVERAGE_INFO}
          COMMAND ${LCOV_PATH} --ignore-errors gcov --directory ${CMAKE_BINARY_DIR} --zerocounters
          COMMAND $<TARGET_FILE:${TARGET_NAME}> ${tesseract_code_coverage_ARGS}
          COMMAND
            ${LCOV_PATH} --ignore-errors gcov --directory ${CMAKE_BINARY_DIR} --base-directory
        ...
        add_custom_target(
          ...
          COMMAND
            ${GENHTML_PATH} -o
            ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${tesseract_code_coverage_COVERAGE_TARGET_NAME}
            ${COVERAGE_INFO}
          DEPENDS ccov-capture-${tesseract_code_coverage_COVERAGE_TARGET_NAME})
      endif()

      add_custom_command(
        TARGET ccov-${tesseract_code_coverage_COVERAGE_TARGET_NAME}
        POST_BUILD
        COMMAND ;
        COMMENT
          "Open ${CMAKE_COVERAGE_OUTPUT_DIRECTORY}/${tesseract_code_coverage_COVERAGE_TARGET_NAME}/index.html in your browser to view the coverage report."
      )

      # AUTO
      if(tesseract_code_coverage_AUTO)
        if(NOT TARGET ccov)
          add_custom_target(ccov)
        endif()
        add_dependencies(ccov ccov-${tesseract_code_coverage_COVERAGE_TARGET_NAME})
      ...
      # ALL
      if(tesseract_code_coverage_ALL)
...
# `tesseract_code_coverage` with the `ALL` parameter, but merges all the coverage
