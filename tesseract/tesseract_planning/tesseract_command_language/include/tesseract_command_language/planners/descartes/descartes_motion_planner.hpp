...
DescartesMotionPlanner<FloatType>::DescartesMotionPlanner(std::string name)
  : MotionPlanner(name)
  , config_(nullptr)
  , status_category_(std::make_shared<const tesseract_motion_planners::DescartesMotionPlannerStatusCategory>(name))
{
}
...
template <typename FloatType>
tesseract_common::StatusCode DescartesMotionPlanner<FloatType>::solve(tesseract_motion_planners::PlannerResponse& response,
                                                                      ...
                                                                      const bool verbose)
{
  ...
  descartes_light::Solver<FloatType> graph_builder(config_->dof);
...
//                 });
    ...
    response.status =
        tesseract_common::StatusCode(tesseract_motion_planners::DescartesMotionPlannerStatusCategory::ErrorFailedToBuildGraph, status_category_);
...
//  response.failed_waypoints.clear();
  ...
  if (!graph_builder.search(solution))
  {
    ...
    response.status = tesseract_common::StatusCode(tesseract_motion_planners::DescartesMotionPlannerStatusCategory::ErrorFailedToFindValidSolution,
                                                   status_category_);
    return response.status;
  }
...
//  }
  ...
  response.status = tesseract_common::StatusCode(tesseract_motion_planners::DescartesMotionPlannerStatusCategory::SolutionFound, status_category_);
  return response.status;
}
...
{
  request_ = tesseract_motion_planners::PlannerRequest();
  config_ = nullptr;
}
...
{
  if (config_ != nullptr)
    return tesseract_common::StatusCode(tesseract_motion_planners::DescartesMotionPlannerStatusCategory::IsConfigured, status_category_);

  return tesseract_common::StatusCode(tesseract_motion_planners::DescartesMotionPlannerStatusCategory::ErrorIsNotConfigured, status_category_);
}
