/**
 * @file raster_global_process_manager.cpp
...
RasterGlobalProcessManager::RasterGlobalProcessManager(TaskflowGenerator::UPtr global_taskflow_generator,
...
bool RasterGlobalProcessManager::init(ProcessInput input)
{
  ...
  global_task_ =
      taskflow_
          .composed_of(global_taskflow_generator_->generateTaskflow(
              input,
              std::bind(&RasterGlobalProcessManager::successCallback, this, input_instruction->getDescription()),
              std::bind(&RasterGlobalProcessManager::failureCallback, this, input_instruction->getDescription())))
          .name("global");
    ...
    auto raster_step =
        taskflow_
            .composed_of(raster_taskflow_generator_->generateTaskflow(
                raster_input,
                std::bind(&RasterGlobalProcessManager::successCallback,
                          this,
                          raster_input.getInstruction()->getDescription()),
                std::bind(&RasterGlobalProcessManager::failureCallback,
                          this,
                          raster_input.getInstruction()->getDescription())))
  ...
  auto to_end =
      taskflow_
          .composed_of(freespace_taskflow_generator_->generateTaskflow(
              to_end_input,
              std::bind(
                  &RasterGlobalProcessManager::successCallback, this, to_end_input.getInstruction()->getDescription()),
              std::bind(
                  &RasterGlobalProcessManager::failureCallback, this, to_end_input.getInstruction()->getDescription())))
...
bool RasterGlobalProcessManager::execute()
{
  success_ = true;
  ...
  if (debug_)
    debug_observer = executor_.make_observer<DebugObserver>("RasterGlobalProcessManagerObserver");
  ...
  {
    std::ofstream out_data;
    out_data.open(tesseract_common::getTempPath() + "raster_global_process_manager_profile-" +
...
bool RasterGlobalProcessManager::terminate()
{
...
bool RasterGlobalProcessManager::clear()
...
void RasterGlobalProcessManager::enableDebug(bool enabled) { debug_ = enabled; }

void RasterGlobalProcessManager::enableProfile(bool enabled) { profile_ = enabled; }

void RasterGlobalProcessManager::globalPostProcess(ProcessInput input)
{
...
bool RasterGlobalProcessManager::checkProcessInput(const tesseract_planning::ProcessInput& input) const
{
  // -------------
  // Check Input
  // -------------
...
void RasterGlobalProcessManager::successCallback(std::string message)
{
  CONSOLE_BRIDGE_logInform("%s", message.c_str());
  success_ &= true;
}

void RasterGlobalProcessManager::failureCallback(std::string message)
{
  ...
  if (success_)
    CONSOLE_BRIDGE_logError("RasterGlobalProcessManager Failure: %s", message.c_str());
  success_ = false;
}
