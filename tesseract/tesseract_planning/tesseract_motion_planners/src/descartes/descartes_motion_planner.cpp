...
bool DescartesMotionPlanner<FloatType>::setConfiguration(const DescartesMotionPlannerConfig<FloatType> &config)
{
  ...
  {
    CONSOLE_BRIDGE_logError("In %s: tesseract is a required parameter and has not been set", name_);
    return false;
  }

  if (config.contact_checker == nullptr)
  {
    CONSOLE_BRIDGE_logError("In %s: contact_checker is a required parameter and has not been set", name_);
    return false;
  }

  if (config.kinematics == nullptr)
  {
    CONSOLE_BRIDGE_logError("In %s: kinematics is a required parameter and has not been set", name_);
    return false;
  }

  if (config.edge_evaluator == nullptr)
  {
    CONSOLE_BRIDGE_logError("In %s: edge_evaluator is a required parameter and has not been set", name_);
    return false;
  }

  if (config.timing_constraint.empty())
  {
    CONSOLE_BRIDGE_logError("In %s: timing_constraint is a required parameter and has not been set", name_);
    return false;
  }

  if (config.waypoints.empty())
  {
    CONSOLE_BRIDGE_logError("In %s: waypoints is a required parameter and has not been set", name_);
    return false;
  }
  ...
  if ((config.timing_constraint.size() != config.waypoints.size() != config.samplers.size()))
  {
    CONSOLE_BRIDGE_logError("In %s: waypoints, timing_constraint and samplers must be the same size", name_);
    return false;
  }
...
tesseract_common::StatusCode DescartesMotionPlanner<FloatType>::solve(PlannerResponse& response)
{
  ...
  const auto dof = config_->tesseract_->getFwdKinematicsManagerConst()->getFwdKinematicSolver(config_->manipulator)->numJoints();
