...
{
OMPLTrajOptFreespacePlanner::OMPLTrajOptFreespacePlanner(std::string name)
  : MotionPlanner(name), status_category_(std::make_shared<tesseract_common::GeneralStatusCategory>(name))
{
}

bool OMPLTrajOptFreespacePlanner::setConfiguration(OMPLPlannerConfig::Ptr ompl_config,
...
bool OMPLTrajOptFreespacePlanner::terminate()
{
  bool success = true;
...
void OMPLTrajOptFreespacePlanner::clear()
{
  ompl_planner_.clear();
  trajopt_planner_.clear();
}

tesseract_common::StatusCode OMPLTrajOptFreespacePlanner::isConfigured() const
{
