 ...
 */
#ifndef TESSERACT_SCENE_GRAPH_URDF_PARSER_LINK_H
#define TESSERACT_SCENE_GRAPH_URDF_PARSER_LINK_H
...
inline tesseract_common::StatusCode::Ptr parse(Link::Ptr& link,
                                               ...
                                               const std::unordered_map<std::string, tesseract_scene_graph::Material::Ptr> available_materials)
{
  ...
  Link::Ptr l = std::make_shared<Link>(link_name);
  ...
  {
    std::vector<Visual::Ptr> temp_visual;
  ...
  {
    std::vector<Collision::Ptr> temp_collision;
...
#endif // TESSERACT_SCENE_GRAPH_URDF_PARSER_LINK_H
