 ...
 */
#ifndef TESSERACT_SCENE_GRAPH_URDF_PARSER_MATERIAL_H
#define TESSERACT_SCENE_GRAPH_URDF_PARSER_MATERIAL_H
...
inline tesseract_common::StatusCode::Ptr parse(Material::Ptr& material,
                                               const tinyxml2::XMLElement* xml_element,
                                               const std::unordered_map<std::string, Material::Ptr>& available_materials)
{
  ...
  Material::Ptr m = std::make_shared<Material>(material_name);
  ...
  if (color != nullptr)
  {
    ...
    if (!color_string.empty())
    {
      ...
      m->color = Eigen::Vector4d(std::stod(tokens[0]), std::stod(tokens[1]), std::stod(tokens[2]), std::stod(tokens[3]));
    }
    else
    {
      return std::make_shared<tesseract_common::StatusCode>(MaterialStatusCategory::ErrorColorAttributeRGBA, status_cat);
...
#endif // TESSERACT_SCENE_GRAPH_URDF_PARSER_MATERIAL_H
