 ...
 */
#ifndef TESSERACT_SCENE_GRAPH_URDF_PARSER_GEOMETRY_H
#define TESSERACT_SCENE_GRAPH_URDF_PARSER_GEOMETRY_H
  ...
  public:
    ...
    std::string message(int code) const override
    {
      switch (code)
      {
        ...
        case ErrorParsingOctree:
          return "Failed parsing geometry type octree!";
                                               ...
                                               bool visual)
{
  ...
  if (geometry == nullptr)
    return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingGeoemtryElement, status_cat);
  ...
  if (geometry_type == "sphere")
  {
    ...
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingSphere, status_cat, status);
  ...
  else if (geometry_type == "box")
  {
    ...
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingBox, status_cat, status);
  ...
  else if (geometry_type == "cylinder")
  {
    ...
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingCylinder, status_cat, status);
  ...
  else if (geometry_type == "cone")
  {
    ...
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingCone, status_cat, status);

    geometries = {cone};
  }
  else if (geometry_type == "octree")
  {
    tesseract_geometry::Octree::Ptr octree;
    tesseract_common::StatusCode::Ptr status = parse(octree, geometry, locator, visual);
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingOctree, status_cat, status);
  ...
  else if (geometry_type == "mesh")
  {
    ...
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingMesh, status_cat, status);
  ...
  else if (geometry_type == "convex_mesh")
  {
    ...
    tesseract_common::StatusCode::Ptr status = parse(meshes, geometry, locator, visual);
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingConvexMesh, status_cat, status);
  ...
  else if (geometry_type == "sdf_mesh")
  {
    ...
    tesseract_common::StatusCode::Ptr status = parse(meshes, geometry, locator, visual);
    if (!(*status))
      return std::make_shared<tesseract_common::StatusCode>(GeometryStatusCategory::ErrorParsingSDFMesh, status_cat, status);
...
#endif // TESSERACT_SCENE_GRAPH_URDF_PARSER_GEOMETRY_H
