...
#include <tesseract_scene_graph/parser/urdf_parser/box.h>
#include <tesseract_scene_graph/parser/urdf_parser/sphere.h>
#include <tesseract_scene_graph/parser/urdf_parser/cone.h>
#include <tesseract_scene_graph/parser/urdf_parser/cylinder.h>
#include <tesseract_scene_graph/parser/urdf_parser/capsule.h>
#include <tesseract_scene_graph/parser/urdf_parser/mesh.h>
#include <tesseract_scene_graph/parser/urdf_parser/convex_mesh.h>
#include <tesseract_scene_graph/parser/urdf_parser/sdf_mesh.h>
#include <tesseract_scene_graph/parser/urdf_parser/octree.h>
#include <tesseract_scene_graph/parser/urdf_parser/calibration.h>
#include <tesseract_scene_graph/parser/urdf_parser/dynamics.h>
#include <tesseract_scene_graph/parser/urdf_parser/inertial.h>
#include <tesseract_scene_graph/parser/urdf_parser/limits.h>
#include <tesseract_scene_graph/parser/urdf_parser/material.h>
#include <tesseract_scene_graph/parser/urdf_parser/mimic.h>
#include <tesseract_scene_graph/parser/urdf_parser/safety_controller.h>
#include <tesseract_scene_graph/parser/urdf_parser/geometry.h>
#include <tesseract_scene_graph/parser/urdf_parser/visual.h>
#include <tesseract_scene_graph/parser/urdf_parser/collision.h>
                                          ...
                                          const std::string element_name)
{
  ...
  auto status = tesseract_scene_graph::parse(type, element);
  if (!(*status))
  {
                                          ...
                                          bool visual)
{
  ...
  auto status = tesseract_scene_graph::parse(type, element, locator, visual);
  if (!(*status))
  {
    CONSOLE_BRIDGE_logError(status->message().c_str());
  }
                                          ...
                                          tesseract_scene_graph::ResourceLocatorFn locator)
{
  ...
  auto status = tesseract_scene_graph::parse(type, element, locator);
  if (!(*status))
  {
    CONSOLE_BRIDGE_logError(status->message().c_str());
  }
                                          ...
                                          const std::unordered_map<std::string, tesseract_scene_graph::Material::Ptr>& available_materials)
{
  ...
  auto status = tesseract_scene_graph::parse(type, element, locator, available_materials);
  if (!(*status))
  {
    CONSOLE_BRIDGE_logError(status->message().c_str());
  }
                                          ...
                                          const std::unordered_map<std::string, tesseract_scene_graph::Material::Ptr>& available_materials)
{
  ...
  auto status = tesseract_scene_graph::parse(type, element, available_materials);
  if (!(*status))
  {
    CONSOLE_BRIDGE_logError(status->message().c_str());
  }
...
TEST(URDF_PARSER_UNIT, parse_origin)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_box)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_sphere)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_cone)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_cylinder)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_capsule)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_mesh)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_sdf_mesh)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_convex_mesh)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_octree)
{
  {
    std::string str = "<octree filename=\"package://tesseract_support/meshes/box_2m.bt\" shape_type=\"box\" scale=\"1 2 1\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_TRUE(*status);
    EXPECT_TRUE(geom->getSubType() == geom->BOX);
    EXPECT_TRUE(geom->getOctree() != nullptr);
    EXPECT_EQ(geom->getOctree()->size(), 121);
  }

  {
    std::string str = "<octree filename=\"package://tesseract_support/meshes/box_2m.bt\" shape_type=\"sphere_inside\" scale=\"1 2 1\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_TRUE(*status);
  ...
  {
    std::string str = "<octree filename=\"package://tesseract_support/meshes/box_2m.bt\" shape_type=\"sphere_outside\" scale=\"1 2 1\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_TRUE(*status);
  ...
  {
    std::string str = "<octree filename=\"package://tesseract_support/meshes/box_2m.bt\" shape_type=\"box\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_TRUE(*status);
    EXPECT_TRUE(geom->getSubType() == geom->BOX);
  ...
  {
    std::string str = "<octree filename=\"abc\" shape_type=\"box\" scale=\"1 2 1\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_FALSE(*status);
  }
  ...
  {
    std::string str = "<octree filename=\"package://tesseract_support/meshes/box_2m.bt\" shape_type=\"star\" scale=\"1 2 1\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_FALSE(*status);
  }
  ...
  {
    std::string str = "<octree filename=\"package://tesseract_support/meshes/box_2m.bt\" shape_type=\"box\" scale=\"1 a 1\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_FALSE(*status);
  }
  ...
  {
    std::string str = "<octree filename=\"package://tesseract_support/meshes/box_2m.bt\" shape_type=\"box\" scale=\"1 2 1 3\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_FALSE(*status);
  }
  ...
  {
    std::string str = "<octree shape_type=\"box\" scale=\"1 2 1\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_FALSE(*status);
  }
  ...
  {
    std::string str = "<octree shape_type=\"box\"/>";
    tesseract_geometry::Octree::Ptr geom;
    auto status = runTest<tesseract_geometry::Octree::Ptr>(geom, str, "octree", locateResource, true);
    EXPECT_FALSE(*status);
  }
...
TEST(URDF_PARSER_UNIT, parse_calibration)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_dynamics)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_inertial)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_limits)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_material)
{
...
TEST(URDF_PARSER_UNIT, parse_mimic)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_safety_controller)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_geometry)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_visual)
{
  std::unordered_map<std::string, tesseract_scene_graph::Material::Ptr> empty_available_materials;
  {
...
TEST(URDF_PARSER_UNIT, parse_collision)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_link)
{
  std::unordered_map<std::string, tesseract_scene_graph::Material::Ptr> empty_available_materials;
  {
...
TEST(URDF_PARSER_UNIT, parse_joint)
{
  {
...
TEST(URDF_PARSER_UNIT, parse_urdf)
{
  {
    std::string str =
        "<robot name=\"test\">"
    ...
    auto status = tesseract_scene_graph::parseURDFString(sg, str, locateResource);
    EXPECT_TRUE(*status);
  ...
  {
    tesseract_scene_graph::SceneGraph::Ptr sg;
    auto status = tesseract_scene_graph::parseURDFFile(sg, std::string(TESSERACT_SUPPORT_DIR) + "/urdf/lbr_iiwa_14_r820.urdf", locateResource);
    EXPECT_TRUE(*status);
  }
}
