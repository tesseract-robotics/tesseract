  ...
  : position(position)
{
  ...
  for (const auto& name : joint_names)
    joint_ids.push_back(JointId(name));
}
...
std::vector<std::string> JointState::getJointNames() const
{
...
JointTrajectory::iterator JointTrajectory::insert(const_iterator p, std::initializer_list<value_type> init_list)
{
  return states.insert(p, init_list);
}
