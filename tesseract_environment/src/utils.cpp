                            ...
                            const tesseract_collision::CollisionCheckConfig& config)
{
  for (const auto& link_name : manager.getActiveCollisionObjects())
    manager.setCollisionObjectsTransform(
        link_name, state0.link_transforms.at(link_name), state1.link_transforms.at(link_name));
                          ...
                          const tesseract_collision::CollisionCheckConfig& config)
{
  ...
  for (const auto& link_name : manager.getActiveCollisionObjects())
    manager.setCollisionObjectsTransform(link_name, state.link_transforms.at(link_name));
