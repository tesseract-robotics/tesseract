...
TEST(TesseractEnvironmentUtils, applyContactManagerConfigObjectEnable)  // NOLINT
{
  ...
  // Check Discrete
  {
    auto collision_check_config = default_collision_check_config;
    ...
    // In collision by default
    {
      ...
      contacts.clear();
      checkTrajectoryState(contacts, *manager, tmap, collision_check_config.contact_request);
  ...
  // Check Continuous
  {
    ...
    {
      contacts.clear();
      checkTrajectorySegment(contacts, *manager, tmap1, tmap2, collision_check_config.contact_request);
      // In collision by default
      EXPECT_FALSE(contacts.empty());
    }
