...
public:
   ...
   */
  ChangeJointAccelerationLimitsCommand(std::unordered_map<std::string, double> limits) : limits_(std::move(limits))
  {
    assert(std::all_of(limits_.begin(), limits_.end(), [](const auto& p) { return p.second > 0; }));
  }

  CommandType getType() const final { return CommandType::CHANGE_JOINT_ACCELERATION_LIMITS; }
