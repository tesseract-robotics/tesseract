...
private:
  ...
  Eigen::Index dof_{ -1 };
  std::size_t grid_size_{ 1 }; /**< @brief Cached product of dof_range_[i].size(); upper-bounds inner-loop iteration
                                  count */
  std::size_t solutions_capacity_hint_{ 0 }; /**< @brief grid_size_ * 8 (OPW upper bound); reserved per IK call to avoid
                                                vector regrowth. */
