...
/**
 * @brief Compute an upper bound on the Cartesian distance from @p base_link_id to @p tip_link_id
 * across all valid joint configurations of the chain.
 *
 * @details Walks the shortest path from @p base_link_id to @p tip_link_id and sums, per joint along
 * the path:
 ...
 *
 * @param scene_graph  Scene graph to query. Must contain both @p base_link_id and @p tip_link_id
 *                     and have a path from one to the other.
 * @param base_link_id Chain start.
 * @param tip_link_id  Chain end.
 ...
 *                   Both bounds must be finite and ordered - joints whose limits were never set
 *                   (or were left infinite) must be given an explicit range by the caller.
 * @param resolution (N,) vector; per-joint maximum step size. Must be finite and > 0 elementwise.
 * @return Vector of N Eigen::VectorXd, each containing the sample points for one joint.
 * @throws std::runtime_error if @p resolution and @p range disagree on the joint count, if any
 *         bound is non-finite or inverted, if any resolution is non-finite or not greater than
 *         zero, or if a range/resolution pair would need more than 10 million samples.
 */
