  ...
  try
  {
    ...
    if (YAML::Node sample_res_node = config["positioner_sample_resolution"])
    {
      sample_res_map = parseSampleResolutionMap(sample_res_node, scene_graph,
                                                "ROPInvKinFactory", "positioner_sample_resolution");
    }
    else
    {
    ...
    {
      auto p_info = parsePluginInfo(positioner, "ROPInvKinFactory", "positioner");
    ...
    {
      auto m_info = parsePluginInfo(manipulator, "ROPInvKinFactory", "manipulator");
