                     ...
                     std::string solver_name)
{
  ...
  if (manipulator->getTipLinkIds().size() != 1)
  ...
  if (tool_positioner->getTipLinkIds().size() != 1)
    throw std::runtime_error("RTPInvKin requires a tool positioner with exactly one tip link");

  const tesseract::common::LinkId manip_tip = manipulator->getTipLinkIds()[0];
  const tesseract::common::LinkId tool_tip = tool_positioner->getTipLinkIds()[0];
  ...
  const double reach = manipulator_reach.has_value() ?
                           *manipulator_reach :
                           computeChainReachUpperBound(scene_graph, manipulator->getBaseLinkId(), manip_tip);
  ...
  const std::vector<Eigen::VectorXd> dof_range =
      buildSampleGrid(tool_sample_range.has_value() ? *tool_sample_range :
                                                      gatherJointLimits(scene_graph, tool_positioner->getJointIds()),
                      tool_sample_resolution);
  ...
  if (manip_tip != tool_base)
  {
    ...
    if (!path.active_joints.empty())
      throw std::runtime_error("Tool positioner base link '" + tool_base.name() +
                               ...
                               "'; found active joint '" + path.active_joints.front().name() + "' on path");
  }
...
tesseract::common::LinkId RTPInvKin::getWorkingFrame() const { return manip_inv_kin_->getBaseLinkId(); }
