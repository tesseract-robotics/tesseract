...
namespace
{
tesseract::common::PropertyTree repInvKinFactorySchema()
{
...
{
tesseract::common::PropertyTree REPInvKinFactory::schema() const { return repInvKinFactorySchema(); }
  ...
  try
  {
    ...
    if (YAML::Node sample_res_node = config["positioner_sample_resolution"])
    {
      for (const auto& entry : sample_res_node)
      {
        ...
        if (jnt == nullptr)
          throw std::runtime_error("REPInvKinFactory, 'positioner_sample_resolution' failed to find joint '" +
...
TESSERACT_SCHEMA_REGISTER(REPInvKinFactory, repInvKinFactorySchema);
TESSERACT_SCHEMA_REGISTER_DERIVED_TYPE(tesseract::kinematics::InvKinFactory, REPInvKinFactory);
