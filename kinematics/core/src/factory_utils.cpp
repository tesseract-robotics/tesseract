                         ...
                         const std::string& block_label)
{
  ...
  // [resolution, min, max] per joint name, before reordering onto the chain's joint order.
  ...
  for (auto it = sample_res_node.begin(); it != sample_res_node.end(); ++it)
  {
    ...
    if (jnt->limits == nullptr)
      throw std::runtime_error(
          tesseract::common::strFormat("%sjoint '%s' has no limits!", block.c_str(), joint_name.c_str()));
                                    ...
                                    const std::string& block_label)
{
  ...
  if (sample_res_map.size() != joint_names.size())
  ...
  {
    const std::string& jn = joint_names[static_cast<std::size_t>(i)];
    auto it = sample_res_map.find(jn);
    if (it == sample_res_map.end())
      throw std::runtime_error(tesseract::common::strFormat("%smissing joint '%s'!", block.c_str(), jn.c_str()));
