/**
 * @file rtp_vs_kdl_nr_jl_benchmarks.cpp
...
/// Compile-time sanity assertions executed before RunSpecifiedBenchmarks.
/// If any assertion trips, the binary aborts and no benchmark data is produced.
void selfCheck(const Fixture& f)
{
  assert(!f.targets.empty());
  ...
  assert(rtp->numJoints() == 7);
  {
    IKSolutions sols;
    rtp->calcInvKin(sols, target0.tip_link_poses, target0.ground_truth);
    assert(!sols.empty() && "RTP should return at least one solution for a reachable target");
#ifndef NDEBUG
  ...
  auto kdl = makeKDLNRJL(f);
  assert(kdl->numJoints() == 7);
  {
    IKSolutions sols;
    kdl->calcInvKin(sols, target0.tip_link_poses, target0.ground_truth);
    assert(!sols.empty() && "KDL-NR-JL should return a solution for a warm-seeded reachable target");
#ifndef NDEBUG
    for (const auto& s : sols)
      assert(isValidSolution(s, full_fk, tool_tip_pose, limits) &&
  ...
  }
  {
    ...
    assert(!sols.empty() && "KDL-NR-JL should converge from a perturbed seed on target[0]; "
