...
public:
  explicit StubFwdKin(std::string base,
    ...
    : base_link_(std::move(base)), joint_name_(std::move(joint)), tip_links_(std::move(tips))
  {
  }
...
TEST(TesseractKinematicsUnit, RTPInvKinIsIndependentOfArmConfiguration)  // NOLINT
{
  ...
  const std::vector<std::string> joint_names = full_fwd_kin.getJointNames();
  ...
  const tesseract::scene_graph::SceneState moved = state_solver.getState(joint_names, q_moved);
...
TEST(TesseractKinematicsUnit, RTPInvKinFactoryFailureMatrix)  // NOLINT
{
  ...
  {  // Non-positive manipulator_reach
    YAML::Node config = tesseract::common::loadYamlString(yaml_str, locator);
    rtpPluginConfig(config)["manipulator_"
                            "reach"] = -1.0;
    load_failure_expected(config);
  }
  ...
  {  // tool_sample_resolution entry missing 'name'
    YAML::Node config = tesseract::common::loadYamlString(yaml_str, locator);
    rtpPluginConfig(config)["tool_sample_"
                            "resolution"][0]
        .remove("name");
    load_failure_expected(config);
  }
  ...
  {  // tool_sample_resolution min greater than max
    YAML::Node config = tesseract::common::loadYamlString(yaml_str, locator);
    auto entry = rtpPluginConfig(config)["tool_"
                                         "sample_"
                                         "resolutio"
                                         "n"][0];
    entry["min"] = 0.5;
  ...
  {  // tool_positioner missing class entry
    YAML::Node config = tesseract::common::loadYamlString(yaml_str, locator);
    auto pos = rtpPluginConfig(config)["too"
                                       "l_"
                                       "pos"
                                       "iti"
                                       "one"
  ...
  {  // manipulator missing class entry
    YAML::Node config = tesseract::common::loadYamlString(yaml_str, locator);
    auto manip = rtpPluginConfig(config)["manipulato"
                                         "r"];
    manip.remove("class");
    load_failure_expected(config);
  }
