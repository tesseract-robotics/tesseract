...
{
  {
    ...
    tesseract::scene_graph::Inertial::Ptr elem;
    EXPECT_TRUE(runTest<tesseract::scene_graph::Inertial::Ptr>(
        elem, &tesseract::urdf::parseInertial, str, tesseract::urdf::INERTIAL_ELEMENT_NAME.data()));
...

  {
    ...
    tesseract::scene_graph::Inertial::Ptr elem;
    EXPECT_FALSE(runTest<tesseract::scene_graph::Inertial::Ptr>(
        elem, &tesseract::urdf::parseInertial, str, tesseract::urdf::INERTIAL_ELEMENT_NAME.data()));
  }
...
{
  {
    tesseract::scene_graph::Inertial::Ptr inertial = std::make_shared<tesseract::scene_graph::Inertial>();
    ...
    inertial->ixz = 6.0;
    std::string text;
    EXPECT_EQ(0, writeTest<tesseract::scene_graph::Inertial::Ptr>(inertial, &tesseract::urdf::writeInertial, text));
    EXPECT_NE(text, "");
  }

  {
    tesseract::scene_graph::Inertial::Ptr inertial = std::make_shared<tesseract::scene_graph::Inertial>();
    inertial->origin = Eigen::Isometry3d::Identity();
    inertial->origin.translation() = Eigen::Vector3d(1.0, 2.0, 3.0);
    inertial->ixx = 1.0;
    inertial->ixy = 2.0;
    inertial->iyy = 3.0;
    inertial->iyz = 4.0;
  ...
  {
    tesseract::scene_graph::Inertial::Ptr inertial = nullptr;
    std::string text;
    EXPECT_EQ(1, writeTest<tesseract::scene_graph::Inertial::Ptr>(inertial, &tesseract::urdf::writeInertial, text));
    EXPECT_EQ(text, "");
  }
}
