...
{
  {
    ...
    EXPECT_TRUE(runTest<Eigen::Isometry3d>(
        origin, &tesseract::urdf::parseOrigin, str, tesseract::urdf::ORIGIN_ELEMENT_NAME.data()));
...
{
  {
    ...
    std::string text;
    EXPECT_EQ(0, writeTest<Eigen::Isometry3d>(origin, &tesseract::urdf::writeOrigin, text));
