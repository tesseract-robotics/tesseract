...
#include <tesseract/collision/bullet/tesseract_compound_collision_algorithm.h>
...
void TesseractCompoundCollisionAlgorithm::preallocateChildAlgorithms(const btCollisionObjectWrapper* body0Wrap,
                                                                     const btCollisionObjectWrapper* body1Wrap)
{
  const btCollisionObjectWrapper* colObjWrap = m_isSwapped ? body1Wrap : body0Wrap;
  ...
  for (int i = 0; i < numChildren; i++)
  {
    ...
    else
    {
      ...
      btCollisionObjectWrapper childWrap(colObjWrap,
                                         childShape,
                                         colObjWrap->getCollisionObject(),
                                         colObjWrap->getWorldTransform(),
                                         -1,
                                         i);  // wrong child trans, but unused (hopefully)
      m_childCollisionAlgorithms[i] =
          m_dispatcher->findAlgorithm(&childWrap, otherObjWrap, m_sharedManifold, BT_CONTACT_POINT_ALGORITHMS);
...
TesseractCompoundCollisionAlgorithm::~TesseractCompoundCollisionAlgorithm() { removeChildAlgorithms(); }

struct TesseractCompoundLeafCallback : btDbvt::ICollide
{
public:
  ...
  TesseractCompoundLeafCallback(const btCollisionObjectWrapper* compoundObjWrap,
  ...
  void ProcessChildShape(const btCollisionShape* childShape, int index)
  {
    ...
    if (TestAabbAgainstAabb2(aabbMin0, aabbMax0, aabbMin1, aabbMax1))
    {
      ...
      {
        algo = m_dispatcher->findAlgorithm(&compoundWrap, m_otherObjWrap, nullptr, BT_CLOSEST_POINT_ALGORITHMS);
...
void TesseractCompoundCollisionAlgorithm::processCollision(const btCollisionObjectWrapper* body0Wrap,
                                                           const btCollisionObjectWrapper* body1Wrap,
                                                           const btDispatcherInfo& dispatchInfo,
                                                           btManifoldResult* resultOut)
{
  ...
  TesseractCompoundLeafCallback callback(colObjWrap,
                                         otherObjWrap,
                                         m_dispatcher,
                                         dispatchInfo,
                                         resultOut,
      ...
      if (m_childCollisionAlgorithms[i] != nullptr)
      {
        ...
        for (int m = 0; m < manifoldArray.size(); m++)
        {
          if (manifoldArray[m]->getNumContacts() != 0)
          {
            ...
            resultOut->setPersistentManifold(nullptr);  //??necessary?
          }
        }
        manifoldArray.resize(0);
      }
...
btScalar TesseractCompoundCollisionAlgorithm::calculateTimeOfImpact(btCollisionObject* body0,
