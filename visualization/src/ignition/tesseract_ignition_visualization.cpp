...
#include <tesseract/visualization/ignition/tesseract_ignition_visualization.h>
...
#include <tesseract/common/class_loader.h>
  ...
  switch (marker.getType())
  {
    ...
    case static_cast<int>(MarkerType::CONTACT_RESULTS):
    {
      ...
      for (size_t i = 0; i < m.dist_results.size(); ++i)
      {
        ...
        double safety_distance = m.margin_data.getPairCollisionMargin(dist.link_ids[0].name(), dist.link_ids[1].name());
