                                                       ...
                                                       const tesseract_scene_graph::ResourceLocator::Ptr& locator)
{
  ...
  if (!sg->isTree())
  {
    if (sg->isAcyclic())
      std::throw_with_nested(
          std::runtime_error("URDF: Error, is not a tree structure for robot '" + robot_name + "'!"));

    std::throw_with_nested(std::runtime_error("URDF: Error, is acyclic structure for robot '" + robot_name + "'!"));
  }
