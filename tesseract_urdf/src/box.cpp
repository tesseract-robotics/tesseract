...
{
tesseract::geometry::Box::Ptr parseBox(const tinyxml2::XMLElement* xml_element)
{
  std::string size_string;
  if (tesseract::common::QueryStringAttribute(xml_element, "size", size_string) != tinyxml2::XML_SUCCESS)
  ...
  tesseract::common::toNumeric<double>(tokens[0], l);
  tesseract::common::toNumeric<double>(tokens[1], w);
  tesseract::common::toNumeric<double>(tokens[2], h);
  ...
  return std::make_shared<tesseract::geometry::Box>(l, w, h);
}

tinyxml2::XMLElement* writeBox(const std::shared_ptr<const tesseract::geometry::Box>& box, tinyxml2::XMLDocument& doc)
{
  if (box == nullptr)
